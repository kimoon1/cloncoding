; (function () {
  ...
  const init = () => {
    document.addEventListener('keydown', (event) => {
      ...
      if (!isDirectionCorrenct(direction)) {
        return;
      }
  ...
  const setHighScore = () => {
    ...
    const finalScore = $scroe.textContent.match(/(\d+)/)[0] * 1;
  ...
  const getFood = () => {
    ...
    if (snakeX == foodX && snakeY == foodY) {
      option.score++;
      $scroe.innerHTML = `점수 : ${option.score}점`;
      setBody();
      randomFood();
    }
  }
  ...
  const isDirectionCorrenct = (direction) => {
    return (
